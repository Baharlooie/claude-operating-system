...
**TDD-for-skills — pressure-test the codification before trusting it:** When codifying a pattern as a new skill file, don't assume the skill will fire correctly. Verify mechanically: (1) construct 2-3 realistic trigger prompts that match the intended use case, (2) dispatch a subagent WITHOUT the skill loaded against those prompts — observe failures or shortcuts, (3) load the skill, re-dispatch — verify the subagent now takes the codified path, (4) iterate the skill's description and triggers until the selection is reliable. A codified skill that doesn't fire is worse than no skill — it gives false confidence that a pattern is captured when it isn't. This pressure-test is especially important when the skill's description overlaps with existing skills (description conflicts cause silent mis-selection). Source: adapted from obra/superpowers' `writing-skills` pattern (reviewed in 2026-04-20 plugin research; plugin itself SKIPPED due to architectural collision, but this specific pattern harvested).

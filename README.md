...
- [**Garry Tan — "Thin Harness, Fat Skills"**](https://x.com/garrytan/status/2042925773300908103) — skill files as parameterized method calls, latent vs deterministic as an explicit design principle, diarization as a named pattern, and the sharpened "if I have to ask twice, you failed" codification trigger
...
- [**Andrej Karpathy's LLM-coding observations via forrestchang/andrej-karpathy-skills**](https://github.com/forrestchang/andrej-karpathy-skills) — coding discipline principles adopted into `/dev`: surgical changes (touch only what you must, every changed line traces to a user request), transform imperative to verifiable goal, and the bias toward minimum code that solves the problem
- [**Harrison Chase — "Your harness, your memory"**](https://blog.langchain.com/your-harness-your-memory/) — the argument that agent harnesses and memory are tightly coupled, and the case for open harnesses and open memory rather than vendor-locked state. Informed the cross-runtime mental model and the PKG project's direction
- [**Trail of Bits**](https://github.com/trailofbits/skills) — the security baseline pattern (credential deny rules, data-exfiltration blocks) and the [devcontainer](https://github.com/trailofbits/claude-code-devcontainer) approach to untrusted-repo isolation. Informed `_foundation/security-baseline.md`
